...
app.controller('TwitterCTRL', function($scope, $http) {
    $scope.searchQuery = "";
		...
		$scope.twitterSearchFunction = function(){
			...
			var xmltype = $scope.checkbox.XML;
			var filename = $scope.fileName;
			console.log(twitterQuery);
			console.log(number);
			
			$.get(address,{track:twitterQuery,number:number,json:jsontype,csv:csvtype,xml:xmltype,filetitle:filename},function(response){ // gets tweets with address var
...
//			});
		...
		$scope.databasePull = function(){
			var address = "http://localhost:3000/dbpull";
			$.get(address);
		};
